...
Run (palm env, from the PALM parent dir):
    python -m PALM.benchmarks.reactions.benchmark_reactions               # sim_threshold=1.0
    python -m PALM.benchmarks.reactions.benchmark_reactions 0.8           # similarity-aware
...
from PALM.benchmarks.common.random_split import random_split_names as random_split

HERE = os.path.dirname(__file__)
OUT = os.path.join(HERE, "..", "results", "hte_reactions_results.csv")
...
def run(name, loader, sim_threshold=1.0):
    ...
    n = len(records)
    t0 = time.time()
    r = split("hypergraph_nd", (records, afm),
              SplitSpec([8, 2], ["train", "test"], seed=42), sim_threshold=sim_threshold)
    assignment = [r.assignment[i] for i in range(len(records))]
    info = r.diagnostics
