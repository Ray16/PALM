...
Run (palm env):  python -m PALM.benchmarks.moleculenet.validate_lowrank
"""
...
def main():
    ...
    for ds in DATASETS:
        ...
        hg_frac, hg_e, hg_m = [], [], []
        for seed in range(3):
            lab = as_labels(run_split("hypergraph", fd,
                                      SplitSpec([8, 2], ["train", "test"], seed=seed, epsilon=EPS),
        ...
        lr = as_labels(run_split("lowrank", fd,
                                 SplitSpec([8, 2], ["train", "test"], seed=0, epsilon=EPS),
                                 rank=256).assignment)
        # DataSAIL live
