...
Run (palm env):  python -m PALM.lowrank_split.nn_leakage_compare
"""
...
def datasail_split(smiles):
    ...
    try:
        e_s, _, _ = datasail(techniques=["C1e"], splits=[8, 2], names=["train", "test"],
                             e_type="M", e_data=data, max_sec=DATASAIL_MAX_SEC)
        return e_s["C1e"][0]
...
def main():
    ...
    for ds in DATASETS:
        ...
        rng.shuffle(rand)
        hg = run_hypergraph_split(fd, [8, 2], ["train", "test"], k=15, preset="quality", seed=0)
        lr = run_lowrank_split(fd, [8, 2], ["train", "test"], rank=256, seed=0)
