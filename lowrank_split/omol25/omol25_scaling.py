...
Run (palm/boltz-2 env):  python -m PALM.lowrank_split.omol25.omol25_scaling
"""
...
def _time(fn, reps):
    """Median wall time of fn() over `reps` CUDA-synced runs. Returns (t_median, last_result)."""
    ts, out = [], None
    for _ in range(reps):
...
def main():
    ...
    run_hypergraph_split({i: _w[i] for i in range(2000)}, [8, 2], ["train", "test"],
        ...
        if m <= HG_MAX_N:
            ...
            for r in range(reps):
                ...
                hg = run_hypergraph_split(fd, [8, 2], ["train", "test"], k=15,
                                          metric="cosine", preset="quality", seed=42 + r)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(RESULTS, "omol25_scaling_time.png"), dpi=150, bbox_inches="tight")
    ...
    ax.legend(); ax.grid(True, which="both", alpha=0.3)
    fig.tight_layout()
    fig.savefig(os.path.join(RESULTS, "omol25_scaling_lpi.png"), dpi=150, bbox_inches="tight")
