...
Run (boltz-2 env, from PALM parent):  python -m PALM.benchmark.benchmark_mcr
...
OUT = os.path.join(os.path.dirname(__file__), "mcr_results.csv")
K = 25
...
def main():
    ...
    t0 = time.time()
    a_ident, _ = run_hypergraph_split_nd(records, afm, [8, 2], ["train", "test"], sim_threshold=1.0)
    t_ident = round(time.time() - t0, 1)
    i_lpi, i_id, _ = evaluate(records, afm, a_ident)

    t0 = time.time()
    a_knn, info = run_hypergraph_split_nd_knn(records, afm, [8, 2], ["train", "test"], k=K)
