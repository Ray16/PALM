...
Run (palm env, from PALM parent):  python -m PALM.benchmark.benchmark_mcr_scaling
...
from ..hypergraph import run_hypergraph_split_nd_knn
...
OUT = os.path.join(os.path.dirname(__file__), "mcr_scaling_results.csv")
K = 25
...
def main():
    ...
    run_hypergraph_split_nd_knn(records_all[:500], afm_all, [8, 2], ["train", "test"], k=K)
    ...
    for n in SIZES:
        ...
        t0 = time.time()
        a_knn, info = run_hypergraph_split_nd_knn(recs, afm_all, [8, 2], ["train", "test"], k=K)
